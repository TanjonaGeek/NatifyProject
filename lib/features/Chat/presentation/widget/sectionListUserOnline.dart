...
class ListUserOnline extends ConsumerWidget {
    ...
    return SizedBox(
      ...
      child: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('users') // Votre collection
            .where('isOnline',
                isEqualTo: true) // Filtre : seulement les utilisateurs en ligne
            .where('uid',
                isNotEqualTo:
                    uidUser) // Filtre : seulement les utilisateurs en ligne
            .snapshots(),
                        ...
                        if (hasStorie) {
                          StorieEntity storie = StorieEntity(
                            ...
                            username: snapshot.data?['data']?.username ??
                                'Utilisateur inconnu',
                            photoUrl: snapshot.data?['data']?.photoUrl != null
                                ? List<Map<String, dynamic>>.from(
                                    snapshot.data!['data'].photoUrl)
                                : [],
                            ...
                            QuivoirStorie:
                                snapshot.data?['data']?.QuivoirStorie != null
                                    ? List<Map<String, dynamic>>.from(
                            ...
                            storyAvailableForUser:
                                snapshot.data?['data']?.storyAvailableForUser !=
                                        null
                                    ? List<String>.from(snapshot
                                        .data!['data'].storyAvailableForUser)
                                    : [],
                          );
                          storieList.add(storie);
                          await Future.delayed(
                          ...
                          Navigator.of(context, rootNavigator: true).push(
                            MaterialPageRoute(
                              builder: (context) => MessageDetail(
                                urlPhoto: data['profilePic']?.toString() ?? "",
