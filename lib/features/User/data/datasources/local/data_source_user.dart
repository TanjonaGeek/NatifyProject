...
abstract class DataSourceUser {
  Future SignIn(String uidUser); // s'authentifier au data source
  Future SignUp(
      String uid, String name, String photoURL); // s'inscrire au data source
  Future SignOut(); // se deconnecter au data source
  Future DeleteAccount(String userId); // supprimer compte au data source
  Future<void> UpdateInfoInAccount(
      String userId,
      String champsName,
      var dataUpdate,
      String flag); // mise a jour information au compte au data source
  ...
  Future<String?>
      getUserTokenNotification(); // recuperer token Notification au data source
