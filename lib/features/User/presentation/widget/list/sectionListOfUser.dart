...
class SectonListOfUser extends ConsumerWidget {
  ...
  const SectonListOfUser({super.key,required this.onTap,required this.notifier});
  ...
  Set<String> subscribedUidsSet = Set<String>.from(subscribedUids.whereType<String>());
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    ...
    if (notifier.nameSearch.isEmpty &&
         ...
         (notifier.rangeOfageDebutAndFin.start.toInt() == 14 && notifier.rangeOfageDebutAndFin.end.toInt() == 90)
          ) {
         return Center(
             ...
             child: Column(
                ...
                children: [
                  SizedBox(
                    width: 130,
                    height: 130,
                    child: Image.asset('assets/earth2.png',color:Theme.of(context).brightness == Brightness.dark ? Colors.white : Colors.black)),
                    // SizedBox(height: 10,),
                    // Text("Aucun résultat".tr,style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20),),
                    SizedBox(height: 10,),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 15),
                      child: Text("Explorez les utilisateurs se trouvant à proximité de votre emplacement.".tr,
                    ...
                    onPressed: (){
                       Navigator.of(context).push(
                                MaterialPageRoute(
                                    builder: (context) => Maps(statusShareDistance: myOwnData.MydataPersiste!.hiddenPosition,photoUrl: myOwnData.MydataPersiste!.profilePic.toString(),)
                              ),
                          );
                    },
                    child:  Column(
    ...
    if (notifier.nameSearch.isNotEmpty) {
      query = query.where('nameParts', arrayContains: notifier.nameSearch.toLowerCase());
    }
    if (notifier.nationaliteGroupSansFlag.isNotEmpty) {
      query = query.where('nationalite', whereIn: notifier.nationaliteGroupSansFlag);
    }
    ...
    query = query
     ...
     .startAt([notifier.rangeOfageDebutAndFin.start.toInt()])
     .endAt([notifier.rangeOfageDebutAndFin.end.toInt()]);
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      ContactCard(
                                        ...
                                        contactSource:data,
                                        onTap: (){
                                           SlideNavigation.slideToPage(context, UserProfileScreen(uid: data['uid']));
                                        },
                                      )
                                    ],
                                  )
