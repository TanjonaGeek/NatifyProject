...
class MapsUserhNotifier extends StateNotifier<MapsUserState> {
  Timer? _debounce;
  late final GeoFlutterFire _geo;
  Stream<List<DocumentSnapshot>>? queryStream;
  final Ref ref;
  MapsUserhNotifier(this.ref) : super(MapsUserState.initial()) {
    _geo = GeoFlutterFire();
    transferDataPrefToNotifer();
  }
  bool get isFetching => state.state != MapsUserConcreteState.loading;
  ...
  Future<void> getCurrentLocation() async {
    if (isFetching) {
      ...
      try {
        ...
        state = state.copyWith(
          isLoading: false,
          state: MapsUserConcreteState.failure,
        );
      }
    }
  }
  ...
  void updateRadius(double newRadius, GoogleMapController controller) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    state = state.copyWith(radius: newRadius, nationaliteCounts: {});
    if (_debounce?.isActive ?? false) _debounce?.cancel();
    double levelZoom = state.zoom;
  ...
  void resetListeNationalite() {
    state = state.copyWith(nationaliteCounts: {});
    print('etat nationalite ${state.nationaliteCounts}');
  }
  ...
  Future<void> searchNearbyUsers() async {
    ...
    try {
      ...
      int ageMin = state.rangeOfageDebutAndFin.start.toInt();
      int ageMax = state.rangeOfageDebutAndFin.end.toInt();
      ...
      String selectedGender = state.sexe;
      ...
      state = state.copyWith(
          nationaliteCounts: nationaliteCounts,
          listAllUserApproximite: listeUserApproximite,
          isLoading: false);
    } catch (e) {
